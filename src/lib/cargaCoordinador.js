...
export async function cargaCrear({ distrito, zona, coordinador, telefono, concejalFijo, concejales }) {
...
// Público (con token): agrega filas (array de {cedula, nombre, telefono, concejal})
export async function cargaAgregar(token, filas) {
  const { data, error } = await supabase.rpc("carga_agregar", { p_token: token, p_filas: filas });
  if (error) throw error;
  return data; // cantidad insertada
}

// Público (con token): envía/cierra la carga
export async function cargaEnviar(token) {
  const { error } = await supabase.rpc("carga_enviar", { p_token: token });
  if (error) throw error;
}
