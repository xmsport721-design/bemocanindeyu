...
export default function AppDirigente({ yaVotaronGlobal, pasoPCGlobal, asignacionesDirigentes, configApp, auth, db }) {
    ...
    if (!ds) {
        return (
            <div className="bg-slate-50 min-h-screen">
                <header style={{ paddingTop: 'calc(env(safe-area-inset-top) + 1rem)' }} className="bg-green-800 text-white p-4 flex justify-between items-center shadow-xl border-b-4 border-green-500 sticky top-0 z-50"><div className="flex items-center gap-3"><span className="bg-green-600 px-2 rounded font-black">BEMO</span><h1 className="text-sm font-bold uppercase">DIRIGENTE</h1></div><button onClick={()=>signOut(auth)} className="text-[10px] bg-green-900 px-3 py-1.5 rounded-full font-black">SALIR</button></header>
    ...
    return (
        <div className="bg-slate-50 min-h-screen pb-20">
            <header style={{ paddingTop: 'calc(env(safe-area-inset-top) + 1rem)' }} className="bg-green-800 text-white p-4 flex justify-between items-center shadow-xl border-b-4 border-green-500 sticky top-0 z-50"><div className="flex items-center gap-3"><span className="bg-green-600 px-2 rounded font-black">BEMO</span><div><h1 className="text-sm font-bold uppercase truncate max-w-[180px]">{ds.nombre}</h1><p className="text-[10px] text-green-200 font-bold uppercase truncate max-w-[180px]">Dirigente de {ds.concejal ? (ds.concejal.includes('-') ? ds.concejal.split('-')[1].trim() : ds.concejal) : ''}</p></div></div><button onClick={salir} className="bg-green-900 p-2 rounded-full"><LogOut size={16}/></button></header>
